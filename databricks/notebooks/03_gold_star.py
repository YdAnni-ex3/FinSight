...
# MAGIC %md
# MAGIC ## Optional: publish Gold to Snowflake (FINSIGHT.ANALYTICS)
# MAGIC Set the `publish_to_snowflake` widget to `true` and add your Snowflake
# MAGIC creds via Databricks **secrets** (scope `finsight`). Uses the built-in
# MAGIC Snowflake Spark connector.

# COMMAND ----------

if dbutils.widgets.get("publish_to_snowflake").lower() == "true":
    sf = {
        "sfURL": f"{dbutils.secrets.get('finsight', 'sf_account')}.snowflakecomputing.com",
        "sfUser": dbutils.secrets.get("finsight", "sf_user"),
        "sfPassword": dbutils.secrets.get("finsight", "sf_password"),
        "sfDatabase": "FINSIGHT",
        "sfSchema": "ANALYTICS",
    ...
    print("Published Gold to Snowflake FINSIGHT.ANALYTICS.")
else:
